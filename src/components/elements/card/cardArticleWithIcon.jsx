...
const CardArticleWithIcon = ({ title, category, description, links }) => {
      return (
                  ...
                  <div>
                        <div className="flex items-center mt-2 text-gray-700 dark:text-gray-200">
                              ...
                              {links.map((link, index) => (
                                    <a key={index} className="mx-2 text-blue-600 cursor-pointer dark:text-blue-400 hover:underline" href={link.url} tabIndex="0" role="link">{link.name}</a>
                              ))}
                        </div>
